...
import { useAddFlightMutation } from "@/redux/flightSlice";

const formSchema = z.object({
  airline: z
    .string()
    .min(3, { message: "Airline Name must have atleast 3 characters" }),
  flight_number: z
    .string()
    .min(3, { message: "Flight number must have atleast 3 characters" }),
  status: z
    .string()
    .min(5, { message: "status must have atleast 5 characters" }),
  ...
  departure_gate: z
    .string()
    .min(3, { message: "Departure gate must have atleast 3 characters" }),
  arrival_gate: z
    .string()
    .min(3, { message: "Arrival gate must have atleast 3 characters" }),
...
const FlightForm = () => {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    addFlight(values).unwrap()
      .then((result) => {
        toast("Flight added successfully");
        console.log(result);
      })
      .catch((error) => {
        toast(`Failed to add flight ${error.data.error}`);
        console.log("Failed to submit flight: ", error);
      });
  }
  return (
      ...
      <section className="space-y-5">
        ...
        <Form {...form}>
          ...
          >
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input
                      placeholder="Enter stops seperate by comma..."
                      {...field}
                      className="w-[280px] py-6"
                    />
                  </FormControl>
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input
                      type="number"
                      placeholder="Enter journy distance..."
                      {...field}
                      className="w-[280px] py-6"
                    />
                  </FormControl>
            ...
            <Button
              type="submit"
              className="border border-solid border-e-inherit w-[280px] py-6 bg-blue-600 hover:bg-blue-700"
            >
              Add
            </Button>
          </form>
